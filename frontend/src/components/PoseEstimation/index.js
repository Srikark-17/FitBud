...
const PoseEstimation = (props) => {
  ...
  const squatDetection = (squatCoords) => {
    ...
    const left = angle(
      squatCoords["leftKnee"].position,
      squatCoords["leftHip"].position,
      squatCoords["rightHip"].position
    );
    const right = angle(
      squatCoords["rightKnee"].position,
      squatCoords["rightHip"].position,
      squatCoords["leftHip"].position
    );
    ...
    if (
      squatsState === "down" &&
      checkSquatStanding(left) &&
      checkSquatStanding(right)
    ) {
      setSquatsState("up");
    } else if (
      squatsState === "up" &&
      checkSquatDown(left) &&
      checkSquatDown(right)
    ) {
      setSquatsState("down");
  ...
  const curlsDetection = (curlCoords) => {
    ...
    if (
      !(
        (leftShoulderCoord && leftElbowCoord && leftWristCoord) ||
        (rightShoulderCoord && rightElbowCoord && rightWristCoord)
      )
    ) {
    ...
    if (leftShoulderCoord && leftElbowCoord && leftWristCoord) {
      ...
      leftConfidence = average([
        leftShoulderCoord.score,
        leftElbowCoord.score,
        leftWristCoord.score,
      ]);
    }
    if (rightShoulderCoord && rightElbowCoord && rightWristCoord) {
      rightConfidence = average([
        rightShoulderCoord.score,
        rightElbowCoord.score,
        rightWristCoord.score,
      ]);
    }
    ...
    const curlAngle = leftSideStatus
      ? angle(
          leftShoulderCoord.position,
          leftElbowCoord.position,
          leftWristCoord.position
        )
      : angle(
          rightShoulderCoord.position,
          rightElbowCoord.position,
          rightWristCoord.position
        );
    ...
    if (
      (curlsState === "" || curlsState === "curl") &&
      checkCurlRest(curlAngle)
    ) {
  ...
  const jjDetection = (jjCoords) => {
    ...
    const jumpingJackPositionStatus =
      leftSide[0].y < leftSide[1].y && rightSide[0].y < rightSide[1].y;
