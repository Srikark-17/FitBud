...
export const average = (listOfNums) =>
  listOfNums.reduce((acc, val) => {
    acc += val;
    return acc;
  }, 0) / listOfNums.length;
...
export const isStanding = (keypoints) => {
  ...
  return (
    Math.abs(rightKnee.position.y - rightHip.position.y) > 20 ||
    Math.abs(leftKnee.position.y - leftHip.position.y) > 20
  );
};
...
export const isSitting = (keypoints) => {
  ...
  let dist1 =
    rightKnee && rightHip
      ? Math.abs(rightKnee.position.y - rightHip.position.y)
      : 0;
  let dist2 =
    leftKnee && leftHip
      ? Math.abs(leftKnee.position.y - leftHip.position.y)
      : 0;
